...
- experimental game-shell branch that replaces the long Dashboard, Quest Hub, Courses, Stories, Coach, Journal, Family Hub, and Help views with fixed-screen mission rooms for laptop and iPad review
...
- Quest Hub with a first-session launch mode and a returning-child resume mode, so first visits are guided and returning sessions stay fast.
...
- `src/components/GameDashboardTab.jsx`, `src/components/GameOverviewTab.jsx`, `src/components/GameCoursesTab.jsx`, `src/components/GameStoriesTab.jsx`, `src/components/GameCoachTab.jsx`, `src/components/GameJournalTab.jsx`, `src/components/GameFamilyTab.jsx`, and `src/components/GameHelpTab.jsx` own the fixed-screen game rooms used on the `codex/kidwiz-game-shell` branch.
...
- `src/lib/supabasePersistence.js` owns optional cloud load/save/delete behavior for signed-in parent workspaces, Learning Studio safety events, and parent-readable JSON exports.
...
7. Supabase values are configured locally for real magic-link login.
8. The KidWiz Supabase project is `fvubmofcaqvrwfmqwric`; migrations in `supabase/migrations/` were applied and smoke-tested on April 29, 2026.
...
- The game-shell branch intentionally hides the regular app chrome on Dashboard, Quest Hub, Courses, Stories, Coach, Journal, Family Hub, and Help so those sections can behave more like KidWiz mission rooms than scrollable website pages.
...
- Dashboard now includes a Parent Outcome Dashboard so parents can see learning readiness, life-skill practice, child momentum, parent clarity, and child-specific next proof points, with direct action links into the most relevant lesson or child view.
...
- Quest Hub now transitions from a first-session launchpad into a quick resume state after weekly progress starts, helping families move from setup to momentum without extra friction.
...
- Supabase persistence is now opt-in and branch-safe: the app can cloud-save signed-in family workspaces after the migration is applied, export a parent-readable family archive, and delete the cloud copy while local demo mode remains the default no-backend review path.
...
- Supabase persistence now exists as an opt-in foundation in the linked KidWiz project, with local frontend Supabase URL/key values configured for signed-in testing.
...
1. Run a real browser magic-link review with the owner's email, then confirm cloud sync appears in the Parent room after sign-in.
