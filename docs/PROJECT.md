...
- homepage and instant-demo Dashboard now lead with a compact Parent Command Center: child status, today’s recommended learning move, safety state, weekly proof, parent attention items, and quick controls before deeper dashboard detail
...
- parent dashboard cockpit with a status strip, main mission card, attention queue, child board, parent controls, and compact Proof, Signals, Plan, Reports, and Readiness panels
...
- `src/components/DashboardTab.jsx` owns the Parent Command Center shell and keeps the longer proof, signal, planning, report, and readiness content inside compact dashboard panels.
...
- The parent dashboard now starts as a command center instead of a long report page, with the main mission and safety state visible before the deeper proof and reporting panels.
